...
?>
<!DOCTYPE html>
<html><head><title>People Images</title>
    ...
    if ($_SERVER['REQUEST_METHOD']=="POST") {
        ...
        while( $actor = $actors->fetchArray() ) {
            if (empty($actor['strThumb']))
                print "<strong>";
            print "<a class='tipsource' href=\"" . $_SERVER['SCRIPT_NAME'] . "/actor/" . urlencode($actor['strActor']) . "\">" . htmlspecialchars($actor['strActor']) . " (" . $actor['cnt'] . ")</a>\n";
            print "<div class='tipdetail'><img src='../movielist.php/Actors/" . rawurlencode($actor['strActor']) . ".jpg' /></div><br/>";
            if (empty($actor['strThumb']))
                print "</strong>";
        }
